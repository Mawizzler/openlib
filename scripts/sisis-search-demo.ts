...
const pickProvider = () => {
  ...
  return providers.find((provider) => provider.api.toLowerCase() === 'sisis') ?? null;
};

const run = async () => {
  const provider = pickProvider();
  if (!provider) {
    console.error('No SISIS provider found in registry.');
    process.exit(1);
  }
...
run().catch((error) => {
  console.error('SISIS demo failed:', error);
  process.exit(1);
});
