...
const pickProvider = () => {
  ...
  return providers.find((provider) => provider.api.toLowerCase() === 'bibliotheca') ?? null;
};

const run = async () => {
  const provider = pickProvider();
  if (!provider) {
    console.error('No Bibliotheca provider found in registry.');
    process.exit(1);
  }
...
run().catch((error) => {
  console.error('Bibliotheca demo failed:', error);
  process.exit(1);
});
