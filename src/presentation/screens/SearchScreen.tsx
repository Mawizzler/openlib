        ...
        <View style={styles.resultTopRow}>
          ...
          <View style={styles.resultContent}>
            ...
            {availability ? (
              ...
              >
                <Text style={styles.badgeText}>{availability.label}</Text>
              </View>
            ) : null}
          </View>
        </View>
    ...
    <ScreenLayout scrollable contentStyle={styles.scrollContent}>
      ...
      {status === 'idle' && !submittedQuery ? (
        <Card
          ...
          body="Search results appear here once you search for a title, author, or keyword."
        />
      ) : null}

      {status === 'success' ? (
        <Card
          title={resultCount > 0 ? `Results for “${submittedQuery}”` : 'No results'}
          body={
            resultCount > 0
              ? `${resultCount} item${resultCount === 1 ? '' : 's'} found.`
              : `No catalog matches found for “${submittedQuery}”.`
          }
        >
          {results ? (
            <FlatList
