...
export class Biber1992Adapter implements LibrarySystemAdapter {
  readonly system = 'biber1992';
  ...
  async search(input: LibrarySystemSearchInput): Promise<OpacSearchResult> {
    ...
    try {
      ...
      const parsed = parseBiber1992SearchResults(html, this.normalizeBaseUrl());
      ...
      console.warn('[Biber1992Adapter] search failed', error);
      return { total: 0, page, pageSize: DEFAULT_PAGE_SIZE, records: [] };
    }
  }
  ...
  async accountLogin(_input: LibraryAccountLoginInput): Promise<LibraryAccountLoginResult> {
    return {
      ...
      message: 'biber1992 account login is not supported in this initial slice.',
    };
  }
  ...
  async fetchAccountSnapshot(): Promise<LibraryAccountSnapshotResult> {
    return {
      status: 'not_supported',
      message: 'biber1992 account snapshots are not supported in this initial slice.',
    };
  }
  ...
  private buildSearchUrl(query: string, page: number) {
    const endpoint = `${this.normalizeBaseUrl()}/search`;
  ...
  private async fetchText(url: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`biber1992 search request failed with HTTP ${response.status}`);
      }
