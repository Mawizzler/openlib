...
export class TouchpointAdapter implements LibrarySystemAdapter {
  readonly system = 'touchpoint';
  ...
  async search(input: LibrarySystemSearchInput): Promise<OpacSearchResult> {
    ...
    try {
      ...
      const parsed = parseTouchpointSearchResults(html, this.normalizeBaseUrl());
      ...
      console.warn('[TouchpointAdapter] search failed', error);
      return { total: 0, page, pageSize: DEFAULT_PAGE_SIZE, records: [] };
    }
  }
  ...
  async accountLogin(_input: LibraryAccountLoginInput): Promise<LibraryAccountLoginResult> {
    return {
      ...
      message: 'touchpoint account login is not supported in this initial slice.',
    };
  }
  ...
  async fetchAccountSnapshot(): Promise<LibraryAccountSnapshotResult> {
    return {
      status: 'not_supported',
      message: 'touchpoint account snapshots are not supported in this initial slice.',
    };
  }
  ...
  private buildSearchUrl(query: string, page: number) {
    const endpoint = `${this.normalizeBaseUrl()}/search.do`;
  ...
  private async fetchText(url: string): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`touchpoint search request failed with HTTP ${response.status}`);
      }
